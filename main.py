...
bfExec = timeit.timeit(stmt="print(bf.bruteforce('A'))", setup=bfSetup, number=repetitions)
dijExec = timeit.timeit(stmt="print(dij.dijkstra('A'))", setup=dijSetup, number=repetitions)

print(f"bfExec runtime: {bfExec * 1000 / repetitions}ms")
print(f"dijExec runtime: {dijExec * 1000 / repetitions}ms")